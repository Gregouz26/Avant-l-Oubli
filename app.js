...
function makeWaveBars(id, count = 36) {
  const bars = [];
  for (let i = 0; i < count; i++) {
    // on génère des barres avec une forme d'onde
...
function renderAudioCard(item) {
  ...
  const barsHtml = bars.map((h, i) => {
    ...
    return `<div class="audio-waveform-bar"
      style="height:${Math.round(h * 100)}%;
             --wave-min:${minH.toFixed(2)};
             --wave-max:${h.toFixed(2)};
             --wave-dur:${dur}s;
             --wave-delay:${del}s"
      data-idx="${i}"></div>`;
  }).join('');
...
function toggleAudio(id) {
  const item = baseAudio.find(a => a.id === id);
  if (!item) return;


  if (audioState.current === id) {
    if (audioState.el.paused) {
      audioState.el.play().catch(() => {});
      startProgressLoop(id);
      document.querySelector(`.audio-card[data-audio-id="${id}"]`)?.classList.add('is-playing');
  ...
  audio.addEventListener('loadedmetadata', () => {
    ...
    if (badge) badge.textContent = fmtTime(audio.duration);
  });
...
function simulatePlayback(id, durationStr) {
  ...
  const parts   = (durationStr || '1:00').split(':').map(Number);
  ...
  const tick = () => {
    ...
    if (elapsed < totalSec && audioState.current === id) {
      audioState.raf = setTimeout(tick, step * 1000);
    } else if (elapsed >= totalSec) {
      stopCurrent();
    }
  };

  audioState.simulating = true;
  audioState.raf = setTimeout(tick, step * 1000);

  const _orig = audioState.el;
  audioState.el = {
    pause: () => { clearTimeout(audioState.raf); },
    paused: false
  };
}
...
function startProgressLoop(id) {
  const loop = () => {
    const audio = audioState.el;
    if (!audio || audioState.current !== id) return;
...
function updateWaveBars(id, pct) {
  ...
  bars.forEach((bar, i) => {
    bar.classList.toggle('played', i < threshold);
  });
}
